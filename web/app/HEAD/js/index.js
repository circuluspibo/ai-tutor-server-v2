...
function start(isStep){
  ...
  if(Date.now() % 2 == 0){
    target = 'left'
    wrong = 'right'
    pibo.extend('head_pan',{ pos : 'vvright' })
  } else {
    target = 'right'
    wrong = 'left'
    pibo.extend('head_pan',{ pos : 'vvleft'})
  }

  console.log(target,wrong)

  $.query(`img.${wrong}`).src = '/app/HEAD/image/hand.png'
  $.query(`img.${target}`).src = `/app/HEAD/image/check.png`
}
