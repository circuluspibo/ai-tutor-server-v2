...
export function create(){
  ...
  fetch('/v1/content/youtube?text=실시간%20건강체조%20교실').then((response) => response.json()).then(data=>{
    const item = data.data[0]
    pibo.tell(item.title + '을 보여줄게요. 잘 따라해 보세요.')
    $.query('iframe').src = `https://www.youtube.com/embed/${item.id}?autoplay=1&t=0`
    $.query('div.title h1').innerText = `(${item.length.simpleText}) ${item.title}`
  }) 
