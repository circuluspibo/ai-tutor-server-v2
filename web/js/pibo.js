...
pibo = {
  ...
  send: (key, value, cb) => {
    ...
    const to = setTimeout(() => {
      if (pibo._callback[key] && pibo._callback[key].cb instanceof Function)
        pibo._callback[key].cb(false);
    }, timeout);

    if(cb)
      pibo._callback[key] = { cb, to }
    else if(pibo._callback[key]) 
...
const funcs = {
  ...
  f_1: ['$_setup', 'env', 'plug','dev','handshake'],
  f_0_cb: [
    'wifi_list',
    'version',
    'compliment',
  ...
  f_0: ['kill', 'stop', 'halt', 'mute','volume_up','volume_down','listen','hotword'],
};
...
window.onload = function () {
  ...
  //require(['https://cdnjs.cloudflare.com/ajax/libs/mqtt/4.3.7/mqtt.min.js'],function(_){
  //  mqtt = _
